...
<body>
  ...
  $rest = [];
  $panjangArray = count($numbers);

  for ($z = 0; $z < $panjangArray; $z++) {
    $rest[$z] = $numbers[$z] % 5;
  }
  ...
  //kode berikut harus saya pelajari lagi, belum begitu paham
  //bongkar membongkar array assosiative
  $pola = [];
  $keyBaru = ['id', 'name', 'price', 'description', 'source'];

  for ($num = 0; $num < count($items); $num++) {
    for ($num1 = 0; $num1 < count($items[$num]); $num1++) {
      $pola[$num][$keyBaru[$num1]] = $items[$num][$num1];
    }
  }
  for ($num = 0; $num < count($pola); $num++) {
    print_r($pola[$num]);
    echo '<br>';
  }
  echo "<br>";
